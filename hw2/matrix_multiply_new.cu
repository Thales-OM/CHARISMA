...
extern "C" void unifiedMultiplyMatrices(MatrixGenFunc matrixGenFunc, int n) {
    ...
    dim3 numBlocks((n + threadsPerBlock.x - 1) / threadsPerBlock.x,
                   (n + threadsPerBlock.y - 1) / threadsPerBlock.y);
    squareMatrixMultiply<<<numBlocks, threadsPerBlock>>>(A, B, C, n);
...
extern "C" void streamingMultiplyMatrices(double *A, double *B, double *C, int n) {
    ...
    cudaEventRecord(start);
    cudaMemcpyAsync(d_A, A, n * n * sizeof(double), cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(d_B, B, n * n * sizeof(double), cudaMemcpyHostToDevice, stream);

    dim3 threadsPerBlock(TILE_SIZE, TILE_SIZE);
    dim3 numBlocks((n + threadsPerBlock.x - 1) / threadsPerBlock.x,
                   (n + threadsPerBlock.y - 1) / threadsPerBlock.y);
    squareMatrixMultiply<<<numBlocks, threadsPerBlock, 0, stream>>>(d_A, d_B, d_C, n);

    cudaMemcpyAsync(C, d_C, n * n * sizeof(double), cudaMemcpyDeviceToHost, stream);
    cudaEventRecord(stop);
